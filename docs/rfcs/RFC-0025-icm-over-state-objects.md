# RFC-0025: ICM over State Objects — Information Context Management as a Native Convention
...
6. **POSIX-overlay parity.** The same conceptual workspace can be expressed on a
   ...
   (github.com/AdamPippert/repo-scaffold); RFC-0025 is the native counterpart.

### 3.2 Non-Goals

- **A workflow execution engine.** Stage execution is RFC-0018 / RFC-0003;
  RFC-0025 only annotates and views.
...
kernel-enforced (preserving DG-8, minimal overhead). RFC-0025 reserves the
...
- **RFC-0002 (State Objects):** supplies identity, metadata, provenance, and
  policy. ICM is a reading of these. RFC-0025 reserves the `anno.icm.*` key
  prefix under §7.4.
- **RFC-0018 (Workflow Objects):** supplies the stage graph and per-node input
  declarations from which *role* is read. RFC-0025 does not duplicate workflow
...
- **RFC-0011 (Agent-Native Utilities):** hosts the `icm` tool in its utility
  catalog (§4.10) and ships it in Wave 2. RFC-0025 supplies the model and the
