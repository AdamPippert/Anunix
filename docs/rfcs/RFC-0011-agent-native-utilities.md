...
The utility catalog (§4) hosts the **`icm`** tool, whose underlying convention — Information Context Management over State Objects — is defined in **RFC-0025**. RFC-0025 supplies the model; this RFC is its userland home.
...
role-scoped context. ICM is a convention defined in **RFC-0025**, not a new
...
userland/
  ...
  decay/
    ...
    Makefile
  icm/
    icm.c               # CLI: catalog, graph, show, tag, published (RFC-0025)
    Makefile
...
3. `rstat`
4. `capctl`
5. `icm` — ICM catalog/graph/tag views (RFC-0025); builds on `objls`/`objprov`
