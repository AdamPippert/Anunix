...
KERNEL="${PROJECT_DIR}/build/x86_64/anunix-qemu.elf"
...
echo "  kernel: $(ls -lh "${ISO_DIR}/boot/anunix.elf" | awk '{print $5}')"
...
# Create EFI system partition image (FAT12, 1.44 MB floppy image)
EFI_IMG="${ISO_DIR}/EFI/BOOT/efiboot.img"

if [ -f "${ISO_DIR}/EFI/BOOT/BOOTX64.EFI" ]; then
	# Create a FAT image using hdiutil (macOS-native)
	EFI_TMP="${BUILD_TMP:-/tmp}/efi_build"
	rm -rf "${EFI_TMP}"
	mkdir -p "${EFI_TMP}/EFI/BOOT"
	cp "${ISO_DIR}/EFI/BOOT/BOOTX64.EFI" "${EFI_TMP}/EFI/BOOT/"

	hdiutil create -srcfolder "${EFI_TMP}" \
		-fs MS-DOS -fsargs "-F 12" \
		-volname EFI \
		-layout NONE \
		"${EFI_IMG%.img}" > /dev/null 2>&1 && {
		# hdiutil creates a .dmg — rename to .img
		mv "${EFI_IMG%.img}.dmg" "${EFI_IMG}" 2>/dev/null || true
		echo "  efiboot.img: $(ls -lh "${EFI_IMG}" | awk '{print $5}')"
	} || {
		echo "  WARNING: Could not create EFI image — UEFI boot may not work"
	}
	rm -rf "${EFI_TMP}"
else
