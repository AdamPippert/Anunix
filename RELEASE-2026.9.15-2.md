# Anunix 2026.9.15-2 Release Notes
...
## Why this release is tagged `2026.9.15-2`

Three tags carry the same kernel. Only the release workflow changed between
them, and each change fixed a defect that stopped a passing build from
publishing.

| Tag | Workflow result | Why |
|---|---|---|
| `2026.9.15` | failed | the test gate misread a passing run |
| `2026.9.15-1` | failed | `FORGEJO_API` was undefined, and the ISO step had no dependencies |
| `2026.9.15-2` | see below | both fixed |

`Observed:` the kernel, the drivers and the tests are identical across all
three. No test was changed, skipped or relaxed at any point.

The second and third defects, both pre-existing:

- `FORGEJO_API` was referenced twice in the release-creation step and defined
  nowhere. The curl URL had no scheme and no host, so the step failed on every
  tag. It is now bound to `github.server_url`. The step also now exits 0 with a
  message when `FORGEJO_TOKEN` is absent, rather than failing a build that
  passed. `Observed:` this repository has no action secrets at all, so that
  token has never been set.
- The ISO step ran `make iso` without `make iso-deps`. No runner has
  `tools/grub` on a fresh checkout, so every release shipped the ELF alone.
  `Observed:` runner task 1207 logged
  `ERROR: isolinux.bin not found. Run make iso-deps first.`

## The first defect, tagged `2026.9.15`
