...
### 2026.4.24 — Native in-kernel browser engine

Anunix ships a **complete HTML/CSS/JS browser engine in the kernel** —
no external process, no Playwright. The engine tokenizes HTML, builds a DOM,
cascades CSS, runs JavaScript on a NaN-boxing VM with mark-sweep GC, decodes
JPEG/PNG/WebP, and renders to a 1280×800 off-screen framebuffer streamed
over WebSocket at ~30 FPS via the ANX-Browser Protocol on port 9191.
HTTPS is reached via a host-side CONNECT proxy.
See [`RELEASE-2026.4.24.md`](RELEASE-2026.4.24.md).
