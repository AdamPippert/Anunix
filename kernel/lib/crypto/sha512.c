 ...
 *
 * Needed by Ed25519. Follows FIPS 180-4.
 */
		       ...
		       uint32_t len)
{
	...
	uint32_t buffered = (uint32_t)(ctx->total & 127);
	uint32_t space;
...
void anx_sha512_final(struct anx_sha512_ctx *ctx, uint8_t out[64])
{
	...
	if (buffered > 112) {
		anx_memset(ctx->buf + buffered, 0, 128 - buffered);
		sha512_compress(ctx->state, ctx->buf);
		buffered = 0;
	}

	anx_memset(ctx->buf + buffered, 0, 112 - buffered);
	/* SHA-512 uses 128-bit length; we only support 64-bit totals */
	anx_memset(ctx->buf + 112, 0, 8);
	store_be64(ctx->buf + 120, total_bits);
	sha512_compress(ctx->state, ctx->buf);
...
void anx_sha512(const void *data, uint32_t len, uint8_t out[64])
{
	...
	anx_sha512_init(&ctx);
	anx_sha512_update(&ctx, data, len);
	anx_sha512_final(&ctx, out);
}
