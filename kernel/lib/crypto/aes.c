/*
 * aes.c — AES-256 in CTR mode.
 *
 * Uses precomputed S-box (no T-tables) to avoid cache-timing attacks.
 * The S-box is constant data, not secret-dependent, so lookups are safe.
 */

#include <anx/types.h>
#include <anx/string.h>
#include <anx/crypto.h>

static const uint8_t sbox[256] = {
	,
};

/* Round constants for key expansion */
static const uint8_t rcon[10] = {
	0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36,
};

/* GF(2^8) multiplication by 2 */
static uint8_t xtime(uint8_t x)
{
	return (uint8_t)((x << 1) ^ (((x >> 7) & 1) * 0x1b));
}

static void sub_bytes(uint8_t state[16])
{
	uint32_t i;

	for (i = 0; i < 16; i++)
		state[i] = sbox[state[i]];
}

static void shift_rows(uint8_t state[16])
{
	uint8_t t;

	/* Row 1: shift left by 1 */
	t = state[1]; state[1] = state[5]; state[5] = state[9];
	state[9] = state[13]; state[13] = t;

	/* Row 2: shift left by 2 */
	t = state[2]; state[2] = state[10]; state[10] = t;
	t = state[6]; state[6] = state[14]; state[14] = t;

	/* Row 3: shift left by 3 */
	t = state[15]; state[15] = state[11]; state[11] = state[7];
	state[7] = state[3]; state[3] = t;
}

static void mix_columns(uint8_t state[16])
{
	uint32_t i;
	uint8_t a, b, c, d, h;

	for (i = 0; i < 16; i += 4) {
		a = state[i]; b = state[i+1]; c = state[i+2]; d = state[i+3];
		h = a ^ b ^ c ^ d;
		state[i]   ^= h ^ xtime(a ^ b);
		state[i+1] ^= h ^ xtime(b ^ c);
		state[i+2] ^= h ^ xtime(c ^ d);
		state[i+3] ^= h ^ xtime(d ^ a);
	}
}

static void add_round_key(uint8_t state[16], const uint8_t *rk)
{
	uint32_t i;

	for (i = 0; i < 16; i++)
		state[i] ^= rk[i];
}

/*
 * AES-256 key expansion: 32-byte key -> 240-byte round keys (15 rounds).
 * Round keys stored column-major matching state layout.
 */
static void aes256_expand_key(const uint8_t key[32], uint8_t rk[240])
{
	uint32_t i;
	uint8_t temp[4];

	/* First 32 bytes are the key itself */
	anx_memcpy(rk, key, 32);

	for (i = 8; i < 60; i++) {
		temp[0] = rk[(i - 1) * 4 + 0];
		temp[1] = rk[(i - 1) * 4 + 1];
		temp[2] = rk[(i - 1) * 4 + 2];
		temp[3] = rk[(i - 1) * 4 + 3];

		if (i % 8 == 0) {
			/* RotWord + SubWord + Rcon */
			uint8_t t = temp[0];
			temp[0] = sbox[temp[1]] ^ rcon[i / 8 - 1];
			temp[1] = sbox[temp[2]];
			temp[2] = sbox[temp[3]];
			temp[3] = sbox[t];
		} else if (i % 8 == 4) {
			/* SubWord only for AES-256 */
			temp[0] = sbox[temp[0]];
			temp[1] = sbox[temp[1]];
			temp[2] = sbox[temp[2]];
			temp[3] = sbox[temp[3]];
		}

		rk[i * 4 + 0] = rk[(i - 8) * 4 + 0] ^ temp[0];
		rk[i * 4 + 1] = rk[(i - 8) * 4 + 1] ^ temp[1];
		rk[i * 4 + 2] = rk[(i - 8) * 4 + 2] ^ temp[2];
		rk[i * 4 + 3] = rk[(i - 8) * 4 + 3] ^ temp[3];
	}
}

/* Encrypt a single 16-byte block with AES-256 (14 rounds) */
static void aes256_encrypt_block(const uint8_t rk[240], const uint8_t in[16],
				 uint8_t out[16])
{
	uint8_t state[16];
	uint32_t round;

	anx_memcpy(state, in, 16);
	add_round_key(state, rk);

	for (round = 1; round < 14; round++) {
		sub_bytes(state);
		shift_rows(state);
		mix_columns(state);
		add_round_key(state, rk + round * 16);
	}

	/* Final round (no MixColumns) */
	sub_bytes(state);
	shift_rows(state);
	add_round_key(state, rk + 14 * 16);

	anx_memcpy(out, state, 16);
}

/* Increment 128-bit counter (big-endian, rightmost byte increments) */
static void ctr_increment(uint8_t ctr[16])
{
	int i;

	for (i = 15; i >= 0; i--) {
		if (++ctr[i] != 0)
			break;
	}
}

void anx_aes256_ctr(const uint8_t key[32], const uint8_t iv[16],
		    void *data, uint32_t len)
{
	uint8_t rk[240];
	uint8_t ctr[16];
	uint8_t keystream[16];
	uint8_t *p = (uint8_t *)data;
	uint32_t i, block_len;

	aes256_expand_key(key, rk);
	anx_memcpy(ctr, iv, 16);

	while (len > 0) {
		aes256_encrypt_block(rk, ctr, keystream);
		block_len = len < 16 ? len : 16;

		for (i = 0; i < block_len; i++)
			p[i] ^= keystream[i];

		p += block_len;
		len -= block_len;
		ctr_increment(ctr);
	}

	anx_memset(rk, 0, sizeof(rk));
	anx_memset(keystream, 0, sizeof(keystream));
}
