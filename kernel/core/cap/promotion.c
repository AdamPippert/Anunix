				...
				bool *promote_out)
{
	...
	if (!promote_out)
		return ANX_EINVAL;
	*promote_out = false;
	if (!trial || trial->n == 0 || trial->n > ANX_PROMOTION_TRIAL_MAX)
		return ANX_EINVAL;
	if (num_candidates_tried == 0)
		num_candidates_tried = 1;

	/* Widen before arithmetic; both scores may span the full signed range. */
	min_margin = (int64_t)trial->candidate_scores[0] - trial->incumbent_scores[0];
	for (i = 1; i < trial->n; i++) {
		int64_t margin = (int64_t)trial->candidate_scores[i] -
				  trial->incumbent_scores[i];
		if (margin < min_margin)
			min_margin = margin;
	}

	required_margin = (int64_t)ANX_PROMOTION_MIN_MARGIN_BASE *
			   num_candidates_tried;
