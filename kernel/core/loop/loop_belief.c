 ...
 * the world model, and stores both the raw observation and its latent as
			   ...
			   anx_oid_t *belief_oid_out)
{
	...
	rc = anx_world_observe(&obs);
	if (rc != ANX_OK) {
		if (anx_world_available()) {
			kprintf("[loop_belief] observe failed (%d)\n", rc);
			return rc;
		}
		/* No world model: continue with zero observation */
		anx_memset(&obs, 0, sizeof(obs));
	}

	/* Step 2: persist the observation */
	if (anx_world_available()) {
		rc = anx_world_observe_store(&obs, &obs_oid);
		if (rc != ANX_OK) {
	...
	/* Step 3: encode into the model's latent */
	rc = anx_world_encode(&obs_oid, &latent_oid);
	if (rc != ANX_OK) {
		/*
		 * No model, or not yet trained; proceed with a
	...
	/*
	 * Phase 3 uncertainty: 0.3 when the model's encoder is live (we have
	 * a real latent), 0.8 without a model (structural run only).
	 * A future pass will refine this using prediction variance across
	 * the full action vocabulary.
	 */
	payload.uncertainty = anx_world_available() ? 0.3f : 0.8f;
