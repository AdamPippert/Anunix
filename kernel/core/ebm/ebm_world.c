 ...
 * current belief latent using anx_world_divergence().  High divergence
...
float ebm_world_consistency(anx_oid_t session_oid, anx_oid_t proposal_oid)
{
	...
	if (!anx_world_available())
		return 0.5f;	/* no world model: neutral energy */
	...
	{
		float d = anx_world_divergence(&pp.latent_oid, &bp.latent_oid);

		if (d < 0.0f)
			return 0.5f;	/* the model reported an error */
		return d;
	}
}
